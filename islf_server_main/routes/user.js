...
async function findMappingByContext(
  code_type,
  company_code,
  branch_code,
  department_code,
  ...
  try {
    console.log("Finding mapping with context:", {
      code_type,
      company_code,
      branch_code,
      department_code,
    ...
    if (filter.includes("D") && department_code) {
      whereClauses.push(
        `(department_code = $${idx} OR department_code IS NULL)`
      );
    ...
    if (filter.includes("ST") && service_type_code) {
      whereClauses.push(
        `(service_type_code = $${idx} OR service_type_code IS NULL)`
      );
...
router.post("/", async (req, res) => {
  ...
  try {
    // Format dates safely
    const safeDateOfBirth = dateOfBirth
      ? new Date(dateOfBirth).toISOString().split("T")[0]
      : null;
    const safeJoiningDate = joiningDate
    ...
    console.log("User creation - Context values:", {
      company_code,
      branch_code,
      department_code,
      employeeId,
    ...
    if (
      ...
      console.log(
        "Looking up mapping for employee code with IT setup validation..."
      );
      const mapping = await findMappingByContext(
        "employeeCode",
        company_code,
        branch_code,
        department_code
    ...
    if (!seriesCode && company_code) {
      console.log(
        "Looking up mapping for employee code with hierarchical matching..."
      );
      ...
      for (const queryObj of hierarchicalQueries) {
        ...
        if (mappingRes.rows.length > 0) {
          ...
          console.log(
            `EMPLOYEE SERIES CODE FROM MAPPING (${queryObj.description}):`,
            seriesCode
          );
          break;
        }
    ...
    if (seriesCode) {
      ...
      if (series.is_manual) {
        ...
        if (!employeeId || employeeId.trim() === "") {
          return res
            .status(400)
            .json({
              error: "Manual employee ID entry required for this series",
            });
        }
        // Check for duplicate employeeId
        const exists = await pool.query(
          "SELECT 1 FROM users WHERE employee_id = $1",
          [employeeId]
        );
      ...
      console.log("Falling back to timestamp code. Conditions not met:", {
        seriesCode,
        company_code,
        branch_code,
        department_code,
