...
router.get("/", async (req, res) => {
  try {
    const { company_code, branch_code, department_code, service_type_code } =
      req.query;
    ...
    if (seriesCode) {
      ...
      if (series.is_manual) {
        ...
        const exists = await pool.query(
          "SELECT 1 FROM vendor WHERE vendor_no = $1",
          [vendor_no]
        );
...
// CREATE multiple vendors (for duplication use-case)
router.post("/duplicate", async (req, res) => {
  ...
  try {
    for (const vendor of vendors) {
      ...
      // Fallback: retrieve seriesCode if missing
      if (!seriesCode && company_code && branch_code && department_code) {
        let mappingRes;
        if (service_type_code) {
          mappingRes = await pool.query(
      ...
      if (seriesCode) {
        ...
        if (seriesResult.rows.length === 0) {
          throw new Error("Number series not found");
        }
        ...
        if (series.is_manual) {
          if (!vendor_no || vendor_no.trim() === "") {
            throw new Error("Manual code entry required for this series");
          }
          ...
          if (exists.rows.length > 0) {
            throw new Error("Vendor No already exists");
          }
        } else {
          const client = await pool.connect();
          try {
            ...
            if (relResult.rows.length === 0) {
              throw new Error("Number series relation not found");
            }
      ...
      // Insert vendor and contacts within a single transaction
      const client = await pool.connect();
      try {
        await client.query("BEGIN");
    ...
    return res.status(201).json({ vendors: insertedVendors });
  } catch (error) {
    console.error(error);
    return res
      .status(400)
      .json({ error: error.message || "Vendor creation failed" });
  }
});
...
router.put("/:id", async (req, res) => {
  ...
  try {
    ...
    const oldResult = await pool.query("SELECT * FROM vendor WHERE id = $1", [
      id,
    ]);
    ...
    try {
      ...
      if (contacts && Array.isArray(contacts)) {
        ...
        await client.query("DELETE FROM vendor_contacts WHERE vendor_id = $1", [
          id,
        ]);
