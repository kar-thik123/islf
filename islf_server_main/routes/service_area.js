...
router.post("/", async (req, res) => {
  try {
    ...
    if (filter.includes("C") && !company_code) {
      ...
      return res
        .status(400)
        .json({ error: "Company code is required based on IT setup" });
    }
    if (filter.includes("B") && !branch_code) {
      console.log("❌ Branch code required by IT setup");
      return res
        .status(400)
        .json({ error: "Branch code is required based on IT setup" });
    }
    if (filter.includes("D") && !department_code) {
      console.log("❌ Department code required by IT setup");
      return res
        .status(400)
        .json({ error: "Department code is required based on IT setup" });
    }
    if (filter.includes("ST") && !service_type_code) {
      console.log("❌ Service type code required by IT setup");
      return res
        .status(400)
        .json({ error: "Service type code is required based on IT setup" });
    }
      ...
      try {
        ...
        if (series.is_manual) {
          ...
          if (relResult.rows.length === 0) {
            ...
            client.release();
            console.log(
              "❌ Number series relation not found for series:",
              seriesCode
            );
            return res
              .status(400)
...
// Update service area
// Dinesh: modifed the query to update based on code instead of ID due to the id undefined
