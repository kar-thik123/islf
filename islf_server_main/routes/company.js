...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const maxCompaniesResult = await pool.query("SELECT value FROM settings WHERE key = 'max_companies'");
    const maxCompanies = maxCompaniesResult.rows.length > 0 ? parseInt(maxCompaniesResult.rows[0].value, 10) : 1;

    const currentCompaniesResult = await pool.query('SELECT COUNT(*) FROM companies');
    const currentCompanies = parseInt(currentCompaniesResult.rows[0].count, 10);

    if (currentCompanies >= maxCompanies) {
      return res.status(400).json({
        error: `Company limit reached. Maximum allowed companies: ${maxCompanies}. Please update IT Setup if needed.`
      });
    }
