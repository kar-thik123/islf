  ...
  try {
    ...
    `;
      ...
      const result = await pool.query(
        "SELECT * FROM master_item ORDER BY id DESC"
      );
      res.json(result.rows);
    }
  } catch (err) {
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    let seriesCode;
    let codeType =
      masterType.toLowerCase() === "cargo type" ? "cargoCode" : "chargeCode";
      ...
      try {
        ...
        if (series.is_manual) {
          ...
          const exists = await client.query(
            "SELECT 1 FROM master_item WHERE code = $1",
            [code]
          );
...
router.put("/:id", async (req, res) => {
  ...
  try {
    const oldResult = await pool.query(
      "SELECT * FROM master_item WHERE code = $1",
      [code]
    );
