...
export class MasterTypeService {
  ...
  create(type: any): Observable<any> {
    return this.http.post<any>(this.apiUrl, this.contextPayload.withContext(type));
  }

  update(id: number, type: any): Observable<any> {
    return this.http.put<any>(`${this.apiUrl}/${id}`, this.contextPayload.withContext(type));
  }
