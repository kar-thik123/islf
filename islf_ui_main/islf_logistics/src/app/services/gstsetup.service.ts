...
export class GstSetupService {
  ...
  create(data: GstRule): Observable<GstRule> {
    return this.http.post<GstRule>(this.apiUrl, this.contextPayload.withContext(data));
  }

  update(id: number, data: GstRule): Observable<GstRule> {
    return this.http.put<GstRule>(`${this.apiUrl}/${id}`, this.contextPayload.withContext(data));
  }
