...
@Component({
  ...
  template: `
    <p-toast></p-toast>
    <div class="card">
      <div class="font-semibold text-xl mb-4">{{this.masterType}} - Master Type</div>
      <p-table
        #dt
        [value]="types"
        dataKey="id"
      ...
      >
        <ng-template pTemplate="caption">
          ...
          >
            <ng-container *appHasPermission="{ module: 'Master Types', subModule: 'Service Area', action: 'write' }">
<button
              pButton
              type="button"
              label="Add Type"
        ...
        <ng-template pTemplate="body" let-type let-rowIndex="rowIndex">
          <tr>
            <td>
              <input pInputText [disabled]="true" [(ngModel)]="type.key" />
            </td>
            <td>
              <ng-container
                *ngIf="type.isNew || type.isEditing; else valueText"
              >
                <div class="flex flex-col">
                  <input
                    pInputText
                    [(ngModel)]="type.value"
                    (ngModelChange)="onFieldChange(type, 'value', type.value)"
                    [ngClass]="getFieldErrorClass(type, 'value')"
                    [ngStyle]="getFieldErrorStyle(type, 'value')"
                    class="w-full"
                  />
                  <small
                    *ngIf="getFieldError(type, 'value')"
                    class="p-error text-red-500 text-xs ml-2"
                    >{{ getFieldError(type, 'value') }}</small
                  >
                </div>
              </ng-container>
            ...
            <td>
              <ng-container
                *ngIf="type.isEditing || type.isNew; else statusText"
              >
                <p-dropdown
                  [options]="statuses"
                  [(ngModel)]="type.status"
...
<button
                ...
                <ng-container *appHasPermission="{ module: 'Master Types', subModule: 'Service Area', action: 'delete' }">
<button
                  *ngIf="type.isNew"
                  pButton
                  icon="pi pi-trash"
...
export class serviceAreaTypeComponent implements OnInit, OnDestroy {
  @Input() filterByKey: string = ''; // Filter types by specific key
  types: any[] = [];
  masterType: string = 'SERVICE_AREA';
  activeTypes: any[] = [];
  ...
  ngOnInit() {
    ...
    this.contextSubscription = this.contextService.context$
      ...
      .subscribe(() => {
        console.log(
          'Context changed in MasterTypeComponent, reloading data...'
        );
        this.refreshList();
      });
  ...
  refreshList() {
    ...
    try {
      ...
      this.masterTypeService.getAllByType(this.masterType).subscribe({
        next: (types) => {
          ...
          this.activeTypes = this.types.filter(
  ...
  addRow() {
    ...
    const newRow = {
      id: null,
      key: this.masterType || 'SERVICE_AREA',
      value: '',
      description: '',
      status: 'Active',
      isEditing: true,
