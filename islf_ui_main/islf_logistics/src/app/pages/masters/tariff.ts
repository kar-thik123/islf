    ...
    <div class="card">
      ...
      >
        ...
        <ng-template pTemplate="body" let-tariff>
          <tr
            [ngClass]="{
              'mandatory-row': tariff.isMandatory,
              'expired-row': getTariffStatus(tariff) === 'Expired'
            }"
          >
            <td>{{ tariff.code }}</td>
        ...
        >
          ...
          <div class="grid grid-cols-12 gap-4 mb-6">
            ...
            <div class="col-span-12 md:col-span-2 flex items-center mt-8 ml-8">
              ...
              <label for="mandatory" class="ml-2 font-semibold"
                >Mandatory</label
              >
            </div>
            <!--<div class="col-span-12 md:col-span-3">
                ...
                <td>
                  <div class="flex gap-2">
                    <p-dropdown
                      ...
                      (ngModelChange)="
                        onFieldChange('basis', selectedTariff.basis)
                      "
                      [ngClass]="getFieldErrorClass('basis')"
                      [ngStyle]="getFieldErrorStyle('basis')"
                      placeholder="Select Basis"
...
export class TariffComponent implements OnInit, OnDestroy {
    ...
    this.tariffService.getAll().subscribe({
      next: (data) => {
        ...
        this.tariffs = data.map((tariff: any) => {
          ...
          mappedTariff.periodStartDateFormatted = startDate
            ? this.formatDateForDisplay(startDate)
            : '';
          mappedTariff.periodEndDateFormatted = endDate
  ...
  saveCharge(index: number) {
    ...
    if (!subCharge.id) {
      ...
      this.tariffService
        .updateCharge(this.selectedTariff.id, subCharge.id, subChargePayload)
        .subscribe({
          next: (response: any) => {
    ...
    data.forEach((row, index) => {
      try {
        ...
        const tariff: any = {
          ...
          freightChargeType:
            row['freightChargeType'] || row['Freight Charge Type'] || '',
          isMandatory:
            row['isMandatory'] ||
            row['Mandatory'] === 'Yes' ||
            row['Mandatory'] === true ||
