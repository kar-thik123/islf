      ...
      >
        ...
        <ng-template pTemplate="body" let-sourceSale>
          <tr>
            <td>
              <ng-container
                *ngIf="sourceSale.isNew || sourceSale.isEditing; else codeText"
              >
                <div class="flex flex-col">
                  <input
                    pInputText
                    [(ngModel)]="sourceSale.code"
                    (ngModelChange)="
                      onFieldChange(sourceSale, 'code', sourceSale.code)
                    "
                  ...
                  />
                  <small
                    *ngIf="getFieldError(sourceSale, 'code')"
                    class="p-error text-red-500 text-xs ml-2"
                    >{{ getFieldError(sourceSale, 'code') }}</small
                  >
                </div>
              </ng-container>
            ...
            <td>
              <ng-container
                *ngIf="sourceSale.isNew || sourceSale.isEditing; else nameText"
              >
                <div class="flex flex-col">
                  <input
                    pInputText
                    [(ngModel)]="sourceSale.name"
                    (ngModelChange)="
                      onFieldChange(sourceSale, 'name', sourceSale.name)
                    "
                    [ngClass]="getFieldErrorClass(sourceSale, 'name')"
                    class="w-full"
                  />
                  <small
                    *ngIf="getFieldError(sourceSale, 'name')"
                    class="p-error text-red-500 text-xs ml-2"
                    >{{ getFieldError(sourceSale, 'name') }}</small
                  >
                </div>
              </ng-container>
            ...
            <td>
              <ng-container
                *ngIf="
                  sourceSale.isNew || sourceSale.isEditing;
                  else commissionText
                "
              >
                <div class="flex flex-col">
                  <p-inputNumber
                    [(ngModel)]="sourceSale.commission_percentage"
                    (onInput)="
                      onFieldChange(
                        sourceSale,
                        'commission_percentage',
                        sourceSale.commission_percentage
                      )
                    "
                    [ngClass]="
                      getFieldErrorClass(sourceSale, 'commission_percentage')
                    "
                    [min]="0"
                    [max]="100"
                    [minFractionDigits]="2"
                  ...
                  ></p-inputNumber>
                  <small
                    *ngIf="getFieldError(sourceSale, 'commission_percentage')"
                    class="p-error text-red-500 text-xs ml-2"
                    >{{
                      getFieldError(sourceSale, 'commission_percentage')
                    }}</small
                  >
                </div>
              </ng-container>
              <ng-template #commissionText
                >{{ sourceSale.commission_percentage }}%</ng-template
              >
            </td>
            <td>
              <ng-container
                *ngIf="sourceSale.isNew || sourceSale.isEditing; else emailText"
              >
                <div class="flex flex-col">
                  <input
                    pInputText
                    [(ngModel)]="sourceSale.email"
                    (ngModelChange)="
                      onFieldChange(sourceSale, 'email', sourceSale.email)
                    "
                    [ngClass]="getFieldErrorClass(sourceSale, 'email')"
                    class="w-full"
                  />
                  <small
                    *ngIf="getFieldError(sourceSale, 'email')"
                    class="p-error text-red-500 text-xs ml-2"
                    >{{ getFieldError(sourceSale, 'email') }}</small
                  >
                </div>
              </ng-container>
            ...
            <td>
              <ng-container
                *ngIf="sourceSale.isNew || sourceSale.isEditing; else phoneText"
              >
                <div class="flex flex-col">
                  <input
                    pInputText
                    [(ngModel)]="sourceSale.phone"
                    (ngModelChange)="
                      onFieldChange(sourceSale, 'phone', sourceSale.phone)
                    "
                    [ngClass]="getFieldErrorClass(sourceSale, 'phone')"
                    class="w-full"
                  />
                  <small
                    *ngIf="getFieldError(sourceSale, 'phone')"
                    class="p-error text-red-500 text-xs ml-2"
                    >{{ getFieldError(sourceSale, 'phone') }}</small
                  >
                </div>
              </ng-container>
...
export class SourceSalesComponent implements OnInit, OnDestroy {
            ...
            this.numberSeriesService.getAll().subscribe({
              next: (seriesList: any[]) => {
                const found = seriesList.find(
                  (s: any) => s.code === this.mappedSourceSalesSeriesCode
                );
                this.isManualSeries = !!(found && found.is_manual);
                console.log(
                  'source sales series code mapped:',
                  this.mappedSourceSalesSeriesCode,
                  'is Manual:',
                  this.isManualSeries
                );
              },
    ...
    if (sourceSale.isNew) {
      this.sourceSalesService.createSourceSales(sourceSaleData).subscribe({
        next: (response: any) => {
          ...
          const index = this.sourceSales.findIndex(
            (item) => item.id === sourceSale.id
          );
          if (index !== -1) {
      ...
      this.sourceSalesService
        .updateSourceSales(sourceSale.id, sourceSaleData)
        .subscribe({
          next: () => {
            this.messageService.add({
              severity: 'success',
  ...
  validateSourceSale(sourceSale: any): boolean {
    ...
    if (
      sourceSale.commission_percentage === null ||
      sourceSale.commission_percentage === undefined
    ) {
      errors.commission_percentage = 'Commission is required';
    } else if (
      sourceSale.commission_percentage < 0 ||
      sourceSale.commission_percentage > 100
    ) {
  ...
  getFieldError(sourceSale: any, field: string): string {
    return sourceSale.errors && sourceSale.errors[field]
      ? sourceSale.errors[field]
      : '';
  }
