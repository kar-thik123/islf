...
export class LockScreen {
    ...
    onUnlock() {
        if (!this.password) {
            this.messageService.add({severity: 'error', summary: 'Unlock Failed', detail: 'Password is required.'});
            return;
        }
        this.loginService.verifyPassword(this.userName, this.password).subscribe({
            next: () => {
                ...
                }, 500); // 0.5 second delay for toast visibility
            },
            error: (err) => {
                this.messageService.add({severity: 'error', summary: 'Unlock Failed', detail: err?.error?.message || 'Incorrect password.'});
            }
        });
    }
} 