import {
  ...
  ɵɵdefineNgModule,
  ɵɵinject
} from "./chunk-VJMBOGEW.js";

// src/environments/environment.ts
var environment = {
  production: false,
  apiUrl: "http://localhost:3001",
  //apiUrl: 'http://3.108.215.65:3001',
  frontendPort: 4200,
...
var CompanyService = class _CompanyService {
  http;
  apiUrl = `${environment.apiUrl}/api/company`;
  constructor(http) {
    this.http = http;
  }
  getAll() {
  ...
  update(code, company) {
    return this.http.put(`${this.apiUrl}/${code}`, company);
  }
  delete(code) {
...
var BranchService = class _BranchService {
  http;
  apiUrl = `${environment.apiUrl}/api/branch`;
  constructor(http) {
    this.http = http;
  }
  getAll() {
  ...
  getByCompany(companyCode) {
    return this.http.get(`${this.apiUrl}?company_code=${companyCode}`);
  }
  getByCode(code) {
    return this.http.get(`${this.apiUrl}/${code}`);
  }
  ...
  update(code, branch) {
    return this.http.put(`${this.apiUrl}/${code}`, branch);
  }
  delete(code) {
    return this.http.delete(`${this.apiUrl}/${code}`);
  }
...
var DepartmentService = class _DepartmentService {
  http;
  apiUrl = `${environment.apiUrl}/api/department`;
  constructor(http) {
    this.http = http;
  }
  getAll() {
  ...
  getByBranch(branchCode) {
    return this.http.get(`${this.apiUrl}?branch_code=${branchCode}`);
  }
  getByCode(code) {
    return this.http.get(`${this.apiUrl}/${code}`);
  }
  ...
  update(code, department) {
    return this.http.put(`${this.apiUrl}/${code}`, department);
  }
  delete(code) {
    return this.http.delete(`${this.apiUrl}/${code}`);
  }
...
// src/app/services/servicetype.service.ts
var ServiceTypeService = class _ServiceTypeService {
  http;
  apiUrl = `${environment.apiUrl}/api/service_types`;
  // Fixed: was /api/number_series
  constructor(http) {
    this.http = http;
  }
  getAll() {
  ...
  getByDepartment(departmentCode) {
    return this.http.get(`${this.apiUrl}/department/${departmentCode}`);
  }
  getByCode(code) {
    return this.http.get(`${this.apiUrl}/${code}`);
  }
  ...
  update(code, serviceType) {
    return this.http.put(`${this.apiUrl}/${code}`, serviceType);
  }
  delete(code) {
    return this.http.delete(`${this.apiUrl}/${code}`);
  }
  static \u0275fac = function ServiceTypeService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _ServiceTypeService)(\u0275\u0275inject(HttpClient));
  };
  static \u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _ServiceTypeService, factory: _ServiceTypeService.\u0275fac, providedIn: "root" });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(ServiceTypeService, [{
    type: Injectable,
    args: [{
      providedIn: "root"
    }]
...
var ContextService = class _ContextService {
  ...
  companyOptions = new BehaviorSubject([]);
  branchOptions = new BehaviorSubject([]);
  departmentOptions = new BehaviorSubject([]);
  serviceTypeOptions = new BehaviorSubject([]);
  // Subject to trigger showing the context selector
  showContextSelectorSubject = new BehaviorSubject(false);
  showContextSelector$ = this.showContextSelectorSubject.asObservable();
  // Public observables that ensure we never emit null
  companyOptions$ = this.companyOptions.asObservable();
  branchOptions$ = this.branchOptions.asObservable();
  departmentOptions$ = this.departmentOptions.asObservable();
  serviceTypeOptions$ = this.serviceTypeOptions.asObservable();
  constructor(injector) {
    this.injector = injector;
  ...
  loadOptions() {
    ...
    companyService.getAll().pipe(map((companies) => companies?.map((c) => ({ label: c.name, value: c.code })) || []), catchError(() => of([]))).subscribe((options) => this.companyOptions.next(options));
    branchService.getAll().pipe(map((branches) => branches?.map((b) => ({ label: b.name, value: b.code })) || []), catchError(() => of([]))).subscribe((options) => this.branchOptions.next(options));
    departmentService.getAll().pipe(map((depts) => depts?.map((d) => ({ label: d.name, value: d.code })) || []), catchError(() => of([]))).subscribe((options) => this.departmentOptions.next(options));
  }
  loadBranchesForCompany(companyCode) {
    const branchService = this.injector.get(BranchService);
    branchService.getByCompany(companyCode).pipe(map((branches) => branches?.map((b) => ({ label: b.name, value: b.code })) || []), catchError(() => of([]))).subscribe((options) => this.branchOptions.next(options));
  }
  loadDepartmentsForBranch(branchCode) {
    const departmentService = this.injector.get(DepartmentService);
    departmentService.getByBranch(branchCode).pipe(map((depts) => depts?.map((d) => ({ label: d.name, value: d.code })) || []), catchError(() => of([]))).subscribe((options) => this.departmentOptions.next(options));
  }
  clearBranchOptions() {
  ...
  loadServiceTypesForDepartment(departmentCode) {
    const serviceTypeService = this.injector.get(ServiceTypeService);
    serviceTypeService.getByDepartment(departmentCode).pipe(map((serviceTypes) => serviceTypes?.map((st) => ({ label: st.name, value: st.code })) || []), catchError(() => of([]))).subscribe((options) => this.serviceTypeOptions.next(options));
  }
  ...
  static \u0275fac = function ContextService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _ContextService)(\u0275\u0275inject(Injector));
  };
...
@angular/common/fesm2022/http.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-CXBDUNMW.js.map
